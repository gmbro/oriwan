...
test("로그인 버튼·기본 복귀·이전 개인창 앵커는 모두 메인으로 돌아온다", () => {
  for (const hash of ["", "#my-activity", "#member-features"]) {
    assert.equal(getSafeAuthReturnPath(`/4th/dashboard${hash}`), "/4th/dashboard");
    assert.equal(getSafeAuthReturnUrl(`/4th/dashboard${hash}`, "https://example.com").href, "https://example.com/4th/dashboard");
  }
  for (const file of ["components/dashboard-gateway-actions.tsx", "app/poc/hello-2027/hello-2027-poc.tsx"]) {
    ...
    assert.doesNotMatch(read(file), /nextPath="\/4th\/dashboard#my-activity|nextPath="\/4th\/dashboard#member-features/);
  }
