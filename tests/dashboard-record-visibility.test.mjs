...
test('spectators receive approved calendar metrics but no pending rows or private fields',()=>{
 const rows=[{recordDateIso:'2026-09-13',status:'certified',distanceKm:5,durationMinutes:30,imageUrl:'private-image',notes:'secret'},{recordDateIso:'2026-09-14',status:'certified',distanceKm:3,durationMinutes:20},{recordDateIso:'2026-09-14',status:'needs_review',distanceKm:99,durationMinutes:99},{recordDateIso:'2026-09-15',status:'certified',distanceKm:88,durationMinutes:88}];
 ...
 assert.deepEqual(p.recordHistory.map(r=>r.recordDateIso),['2026-09-13','2026-09-14']);assert.equal(p.recordHistory.some(r=>'imageUrl' in r||'notes' in r),false);assert.equal(p.distanceKm,null);assert.equal(p.durationMinutes,null);assert.equal('privateFutureField' in p,false);assert.equal(p.product.description,'');assert.deepEqual(out.guestbook,[]);
