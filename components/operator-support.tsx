...
export function OperatorSupport(){
 const status=useSupportStatus(); const [saving,setSaving]=useState(false); const [error,setError]=useState("");
 const complete=async()=>{setSaving(true);try{const r=await fetch("/api/support",{method:"POST"});if(!r.ok)throw new Error();window.dispatchEvent(new Event("twtt:support-updated"));setSelected(null);}catch{setError("저장하지 못했어요. 다시 시도해주세요.");}finally{setSaving(false);}};
 ...
 {(selected=== "bank"||selected==="coffee")&&<div className={styles.detail}><p>후원을 마쳤다면 아래 버튼을 눌러주세요. 완료 표시일부터 한 달 동안 후원이 쉬어갑니다.</p><button type="button" className={styles.action} onClick={()=>void complete()}>{saving?"저장 중…":"후원 완료했어요"}</button></div>}
 </fieldset>}<p role="status">{error||status.error}</p></section>;
}
