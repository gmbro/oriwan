...
export function OperatorSupport({status:initialStatus}:{status?:ReturnType<typeof useSupportStatus>}={}){
 ...
 <button type="button" aria-expanded={selected==='bank'} onClick={()=>setSelected(selected==='bank'?null:'bank')} className={styles.choice}><span className={styles.icon}><ActivityIcon kind="infrastructure"/></span><span><strong>광고 후원</strong></span></button>
