...
export function OperatorSupport(){
 ...
 return <section className={styles.support} aria-labelledby="support-title"><h2 id="support-title">함께 이어가는 TWTT</h2><p className={styles.intro}>후원은 자유롭게 선택할 수 있어요. 후원 여부는 인증 승인과 서비스 이용에 영향을 주지 않아요. 보내주신 응원은 서버 운영과 인증 관리, 커뮤니티 운영에 보탬이 됩니다.</p><div className={styles.choices}>
 ...
 {cheered&&<p role="status" className={styles.feedback}>응원해주셔서 고마워요!</p>}
 ...
 {selected==='coffee'&&<div className={styles.detail}><h3>커피 한 잔으로 응원해주세요</h3><p>카카오 선물하기에서 받는 사람을 직접 선택해야 해요. 운영자 이경민과 연결된 카카오톡 친구인지 확인해주세요.</p><p>운영자를 모르는 경우에는 아래 계좌 후원을 이용해주세요.</p><a href="https://gift.kakao.com" target="_blank" rel="noopener noreferrer" className={styles.action}>카카오 선물하기 열기 <span className="sr-only">새 창</span></a><small>운영자가 자동으로 수신 지정되는 링크는 아니에요.</small></div>}
 <button type="button" aria-expanded={selected==='bank'} onClick={()=>setSelected(selected==='bank'?null:'bank')} className={styles.choice}><span className={styles.icon}><ActivityIcon kind="infrastructure"/></span><span><strong>인프라 비용 지원하기</strong><small>10,000원 · 1회 자율 후원</small></span></button>
 {selected==='bank'&&<div className={styles.detail}><h3>10,000원으로 운영을 응원해주세요</h3><dl><div><dt>은행</dt><dd>신한은행</dd></div><div><dt>계좌번호</dt><dd className={styles.account}>110-489-915994</dd></div><div><dt>예금주</dt><dd>이경민</dd></div></dl><button type="button" className={styles.action} onClick={()=>void copy()}>계좌번호 복사</button><p role="status">{copyStatus}</p><small>은행 앱에서 직접 이체해주세요. 자동결제나 자동 출금은 없으며 계좌 복사만으로 송금되지 않아요.</small></div>}
 </div></section>;
}
