...
export function QuickCertification({today,onSaved}:{today:string;onSaved:()=>void}){
 ...
 <div className={styles.certificationGuide}><p><strong>인증 기준</strong><span>사진 속 운동 시작이 오전 8시 전이고 3km 이상이면 인증돼요. 그 외는 개인 기록으로 저장해요.</span></p><p><strong>업로드 안내</strong><span>시간 제한 없이 여러 번 올릴 수 있어요. 인증일과 보상은 하루 한 번만 인정돼요.</span></p></div>
