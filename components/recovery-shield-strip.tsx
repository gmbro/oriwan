import { RECOVERY_CERTIFICATION_LIMIT } from "@/lib/run-records";

export function RecoveryShieldStrip({
  usedCount,
  className = "",
}: {
  usedCount: number;
  className?: string;
}) {
  const used = Math.max(0, Math.min(Math.floor(usedCount), RECOVERY_CERTIFICATION_LIMIT));

  return (
    <span
      className={`inline-flex shrink-0 items-center gap-0.5 ${className}`.trim()}
      aria-label={`리커버리 쉴드 ${used}/${RECOVERY_CERTIFICATION_LIMIT}회 사용`}
    >
      {Array.from({ length: RECOVERY_CERTIFICATION_LIMIT }, (_, index) => {
        const isUsed = index < used;
        return (
          <span
            key={index}
            className={`inline-flex h-[18px] w-4 items-center justify-center text-[8px] font-black leading-none [clip-path:polygon(50%_0%,91%_14%,82%_72%,50%_100%,18%_72%,9%_14%)] ${
              isUsed
                ? "bg-slate-300 text-slate-600 grayscale"
                : "bg-lime-300 text-slate-950 shadow-sm shadow-lime-300/30"
            }`}
            aria-hidden="true"
          >
            R
          </span>
        );
      })}
    </span>
  );
}
