...
export function SeasonSchedule({ today }: { today: string }) {
  ...
  const daily = items.filter(item => item.date === selected && (!eventId || item.id === eventId));
  ...
  return <section id="schedule" className={styles.crewSection} aria-labelledby="schedule-title">
    ...
    <div className="rounded-3xl bg-white p-4 shadow-sm sm:p-6">
      ...
      <div className="grid grid-cols-7 border-t border-slate-200">{Array.from({length:first.getUTCDay()},(_,i)=><div className="min-h-24 border-b border-slate-100 sm:min-h-32" key={`blank-${i}`}/>)}{Array.from({length:days},(_,i)=>{
        ...
        return <div key={date} className={`min-w-0 border-b border-slate-100 px-0.5 pb-2 sm:px-2 ${selected===date?"bg-blue-50/60":""}`}><button type="button" aria-label={`2026년 ${Number(month.slice(5))}월 ${i+1}일${date===today?", 오늘":""}, 일정 ${events.length}개`} aria-pressed={selected===date} aria-current={date===today?"date":undefined} onClick={()=>{setSelected(date);setEventId(null);if(events.length) setOpened(true);}} className={`my-1 flex h-9 w-full items-center justify-center rounded-xl text-sm ${date===today?"bg-blue-600 font-bold text-white":"text-slate-700"}`}>{i+1}</button><div className="min-h-12 space-y-1 sm:min-h-20">{events.map(event=><button type="button" key={event.id} aria-haspopup="dialog" onClick={()=>{setSelected(date);setEventId(event.id);setOpened(true);}} className="block w-full text-left break-words rounded-md bg-blue-100 px-1 py-1.5 text-[10px] font-semibold leading-4 text-blue-700 sm:px-2 sm:text-xs" title={event.title}>{event.title}</button>)}</div></div>;
      })}</div>
      ...
      <dialog ref={dialog} className={modalStyles.dialog} aria-labelledby="schedule-dialog-title" onCancel={e=>{e.preventDefault();setOpened(false);}} onClose={()=>setOpened(false)} onClick={e=>{if(e.target!==e.currentTarget)return;const r=e.currentTarget.getBoundingClientRect();if(e.clientX<r.left||e.clientX>r.right||e.clientY<r.top||e.clientY>r.bottom)setOpened(false);}}>
        ...
        <div className={modalStyles.body}>{daily.map(event=><article key={event.id}><h3>{event.title}</h3><dl><div><dt>시간</dt><dd>{event.time||"추후 안내"}{event.endTime?` ~ ${event.endTime}`:""}</dd></div><div><dt>장소</dt><dd>{event.location||"추후 안내"}</dd></div></dl>{event.description&&<p>{event.description}</p>}</article>)}</div>
      </dialog>
    </div>
  </section>;
}
