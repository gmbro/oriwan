        ...
        <div className="rounded-[26px] bg-white space-y-7 p-5 text-sm leading-7 text-oriwan-text-muted shadow-sm ring-1 ring-slate-950/5 sm:p-8 sm:text-[15px]">
          ...
          <section>
            ...
            <ul className="list-disc space-y-1 pl-5">
              ...
              <li><strong>비로그인 크루 응원:</strong> 브라우저 식별용 익명 쿠키(1년), 해시 처리한 식별값, 응원 날짜·종류·생성 시각. 하루 중복 응원 방지에 사용하며 이름이나 연락처를 수집하지 않습니다.</li>
              <li><strong>100일 목표 설정:</strong> 목표, 선택 입력한 다짐과 생성 시각. 로그인한 본인과 담당 운영자가 조회·수정 가능</li>
              <li><strong>오늘의 운세 입력:</strong> 이용자가 직접 입력한 이름, 생년월일, 태어난 시간, 생활 권역. 재입력 없이 이용할 수 있도록 본인 계정의 비공개 저장소에 설정과 날짜별 운세 결과를 보관</li>
              <li><strong>오늘의 운세 외부 처리:</strong> 원문 대신 별자리·띠·출생 시간대·광역 생활 권역·비가역 이름 수리 지표만 Google Gemini에 전송</li>
          ...
          <section>
            ...
            <ul className="list-disc space-y-1 pl-5">
              ...
              <li>연결된 참가자가 홈에서 확인할 개인 목표와 다짐 보관</li>
          ...
          <section>
            ...
            <p className="mt-2">
              오늘의 운세 설정과 날짜별 결과는 본인 계정의 비공개 저장소에 보관하며, 로그인한 본인에게만 제공합니다. 외부
              생성 서비스에는 원문 이름·생년월일·태어난 시간을 보내지 않고, 서버에서 만든 비식별 운세 조건만 전송합니다.
              같은 조건의 당일 결과는 저장된 결과와 서명된 HttpOnly 쿠키로 재사용합니다. 외부 API 남용을 막기 위해 사용자별 날짜와
              새 결과 생성 횟수도 Supabase에 보관합니다. 설정·결과·이용량은 탈퇴 또는 운영자를 통한 삭제 요청 시 정리하며, 4기 운영 종료 후 운영 기록 정리 시 삭제합니다.
            </p>
            <p className="mt-2">
              100일 목표 설정의 목표 본문은 서버 전용 저장소에 보관하며, 작성자 본인과 담당 운영자의 인증된 API 응답에만 포함합니다. 담당 운영자는 크루 프로필에서 수정·삭제할 수 있고, 다른 멤버에게는 공개하지 않습니다. 이용자는 운영자에게 삭제를 요청할 수 있으며, 4기 운영 기록 정리 시
              함께 삭제합니다.
            </p>
            <p className="mt-2">
          ...
          <section>
            ...
            <ul className="list-disc space-y-1 pl-5">
              ...
              <li>목표 본문은 작성자 또는 담당 운영자의 인증과 권한을 확인한 API에서만 반환합니다.</li>
              <li>운세 설정과 결과는 서버 전용 비공개 저장소에 보관하고, 원문을 로그에 남기거나 외부 생성 서비스에 보내지 않습니다. 비식별 파생 조건만 외부로 보냅니다.</li>
