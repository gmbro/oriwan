...
- `CORRECTIVE_EXERCISE_LIVE` (문의 저장·파기와 운영 권한 실검증 후에만 `true`)
...
운세 입력 원문은 저장·로깅하거나 Google에 보내지 않고, 서버에서 만든 별자리·띠·출생 시간대·광역 생활 권역·비가역 이름 지표만 Gemini에 전달합니다. 댓글은 기본 활성 상태이며 `HELLO_2027_COMMENTS_DISABLED=true` 또는 기존 `HELLO_2027_COMMENTS_LIVE=false`로 긴급 중단할 수 있습니다. 교정운동 문의는 간단 문의 마이그레이션과 파기 Cron, 운영 권한을 실검증한 뒤 `CORRECTIVE_EXERCISE_LIVE=true`로 명시할 때만 활성화됩니다.
...
기존 일정·문진형 폼을 간단 문의로 전환할 때는 이어서
[`docs/migrations/2026-09-06-corrective-exercise-simple-inquiry.sql`](docs/migrations/2026-09-06-corrective-exercise-simple-inquiry.sql)을 적용합니다.
한 회원이 여러 간단 문의를 남길 수 있도록 하려면 이어서
[`docs/migrations/2026-09-07-corrective-exercise-repeat-inquiries.sql`](docs/migrations/2026-09-07-corrective-exercise-repeat-inquiries.sql)을 적용합니다.
100일 목표 타임머신을 사용하려면
[`docs/migrations/2026-09-07-time-machine-goals.sql`](docs/migrations/2026-09-07-time-machine-goals.sql)을 적용합니다. 목표 본문은 서버가 2027년 1월 1일 00:00(KST) 이후에만 반환합니다.
