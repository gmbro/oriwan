...
공개 댓글·답글·반응 API와 관리자 숨김·복구·비식별 삭제는 구현돼 있다. 작성은 Kakao 세션을 필수로 확인하고, 공개 익명 모드도 계정에서 만든 비가역 actor key를 남용 방지에 사용한다. 본문은 서버와 DB에서 150자로 제한하며 프로세스 단위 제한과 DB의 actor별 1분·1일 제한을 함께 적용한다. 저장소나 권한이 준비되지 않으면 빈 목록과 점검 상태로 실패-폐쇄한다. 긴급 중단은 `HELLO_2027_COMMENTS_DISABLED=true`로 설정하며, 기존 배포의 `HELLO_2027_COMMENTS_LIVE=false`도 계속 존중한다.
