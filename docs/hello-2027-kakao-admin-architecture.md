...
3. 자동 등록된 4기 참가자가 오늘 인증을 완료한 경우 오늘의 응원 상자 개봉
...
| `/api/hello-2027/viewer` | `GET` | Kakao 로그인, 공통 대시보드 참가자 자동 등록, 운영자 확인 표시명 상태 조회 |
...
- 최초 Kakao 로그인은 별도 승인 없이 공통 대시보드에 표시되는 4기 참가자 행을 만들고 개인 화면으로 바로 연결한다.
- 자동 등록 멤버는 운영자가 정렬하기 전까지 관리 멤버 뒤에 배치하며, `/admin` 크루프로필 탭에서 표시 순서와 `display_name_override`를 관리한다.
...
- 같은 이름만으로 기존 멤버에 자동 연결하지 않는다. 인증된 Kakao user ID마다 서버가 결정적 참가자 ID를 사용해 중복 등록을 막는다.
...
| 멤버프로필 | 멤버 등록·수정, 자기소개, 카카오 계정 연결, 운영자 확인 표시명 | 자동 공개 등록·표시 순서·표시명 API 연결 |
...
- 카카오 로그인 계정의 공통 대시보드 참가자 자동 등록, 표시 순서와 표시명 override
